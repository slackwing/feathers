...
function awcml(config, text, configOnly = true) {
  ...
  text = text.replace(re, function (match, op1, args1, op2, op3, op4, op5) {
    if (op1 !== undefined) {
      log("processing @@");
      ...
      log("processing [[]]");
      return parseMacro(config, op3, asyncInstances);
...
function parseMacro(config, input, asyncInstances) {
  indent();
  ...
  if (!async) {
    ...
    dynamicallyLoadMacro(macro);
    return ___macros[macro]({...config}, ...args);
  }
...
async function main() {
  try {
    ...
    document.querySelectorAll(".awcml, script[type='text/awcml']").forEach(function (element) {
      ...
      element.innerHTML = awcml(___config, element.innerHTML, element.tagName == "SCRIPT");
    });
  } catch (e) {
